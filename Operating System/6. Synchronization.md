...
- 2개의 프로세스가 wait()와 signal()을 semaphore에서 동시에 실행하지 않음을 보장해야 한다.
- 따라서, 세마포어 로직을 Critical Section에서 구현해야 한다.
  - 하지만 이렇게 되면 Critical Section의 로직이 길어져서 다른 프로세스에서 Spin Lock이 발생할 수 있다.
- 애플리케이션에서의 Critical Section에서 많은 시간이 걸려 좋은 방법이 아니다.

### Semaphore Implementation with no Busy waiting
- 각 세마포어에는 Waiting Queue가 존재한다.
- Waiting Queue의 각 항목에는 다음과 같은 두 가지 데이터 항목이 있다.
  - (유형 정수의) 값
  - 목록의 다음 레코드를 가리키는 포인터
- 두 가지 작업
  - block – 작업을 호출하는 프로세스를 적절한 대기열에 배치한다.
  - wake up – 대기열에 있는 프로세스 중 하나를 제거하고 준비 대기열에 배치한다.
