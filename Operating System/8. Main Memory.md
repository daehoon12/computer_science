...
## Fragmentation  
- External Fragmentation (외부 단편화) : Allocation 요청에 충분한 Hole이 있으나, 연속적이지 않게 있다.  - Internal Fragmentation (내부 단편화) : Allocation 요청에 충분한 Hole이 없어 Memory를 사용할 수 없음.  - **External Fragmentation**을 줄이기 위해 **Compaction (압축)**을 사용한다. 한쪽으로 다 옮겨서 넓은 Hole을 만듦.
